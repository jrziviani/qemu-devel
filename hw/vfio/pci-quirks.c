...
static void vfio_pci_npu2_atsd_get_link_speed(Object *obj, Visitor *v,
                                              const char *name,
                                              void *opaque, Error **errp)
{
...
int vfio_pci_nvlink2_ram_init(VFIOPCIDevice *vdev, Error **errp)
{
    ...
    struct vfio_region_info_cap_nvidia_nvlink2 *cap;
    ...
    hdr = vfio_get_region_info_cap(nv2region,
                                   VFIO_REGION_INFO_CAP_NVIDIA_NVLINK2);
    cap = (struct vfio_region_info_cap_nvidia_nvlink2 *) hdr;
...
int vfio_pci_npu2_atsd_init(VFIOPCIDevice *vdev, Error **errp)
{
    ...
    hdr = vfio_get_region_info_cap(atsd_region, VFIO_REGION_INFO_CAP_IBM_NPU2);
    cap = (struct vfio_region_info_cap_ibm_npu2 *) hdr;

    object_property_add(OBJECT(vdev), "tgt", "uint64",
                        vfio_pci_npu2_atsd_get_tgt, NULL, NULL,
                        (void *) cap->tgt, NULL);
    object_property_add(OBJECT(vdev), "link_speed", "uint32",
                        vfio_pci_npu2_atsd_get_link_speed, NULL, NULL,
