...
typedef struct SpaprVioVtyDevice {
    SpaprVioDevice sdev;
    CharBackend chardev;
    uint32_t in, out;
...
#define VIO_SPAPR_VTY_DEVICE(obj) \
     OBJECT_CHECK(SpaprVioVtyDevice, (obj), TYPE_VIO_SPAPR_VTY_DEVICE)

static int vty_can_receive(void *opaque)
{
    SpaprVioVtyDevice *dev = VIO_SPAPR_VTY_DEVICE(opaque);
...
{
    SpaprVioVtyDevice *dev = VIO_SPAPR_VTY_DEVICE(sdev);
    int n = 0;
...
static Property spapr_vty_properties[] = {
    DEFINE_SPAPR_PROPERTIES(SpaprVioVtyDevice, sdev),
    DEFINE_PROP_CHR("chardev", SpaprVioVtyDevice, chardev),
...
static const VMStateDescription vmstate_spapr_vty = {
    ...
    .fields = (VMStateField[]) {
        VMSTATE_SPAPR_VIO(sdev, SpaprVioVtyDevice),

        VMSTATE_UINT32(in, SpaprVioVtyDevice),
        VMSTATE_UINT32(out, SpaprVioVtyDevice),
        VMSTATE_BUFFER(buf, SpaprVioVtyDevice),
        VMSTATE_END_OF_LIST()
    },
};
...
static const TypeInfo spapr_vty_info = {
    ...
    .instance_size = sizeof(SpaprVioVtyDevice),
